...
public final class Stage5Killer {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (var entry : peerIDtoAddress.entrySet()) {
            ...
            if (entry.getKey() == 0L) {
                GatewayServer gateway = new GatewayServer(GATEWAY_HTTP_PORT, address.getPort(), 0L, map, 1);
                new Thread(() -> gateway.start(), "Gateway").start();
            } else {
                ZooKeeperPeerServerImpl server = new ZooKeeperPeerServerImpl(address.getPort(), 0, entry.getKey(), map, 0L, 1);
