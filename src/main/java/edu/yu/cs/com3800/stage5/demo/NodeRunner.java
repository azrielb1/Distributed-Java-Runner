...
public class NodeRunner {
    ...
    protected static final int GATEWAY_HTTP_PORT = 9001;
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (thisId == 0L) {
            ...
            new GatewayServer(GATEWAY_HTTP_PORT, address.getPort(), 0L, peerIdToAddress, 1).start();
        } else {
            System.out.println("Starting server " + thisId);
            new ZooKeeperPeerServerImpl(address.getPort(), 0, thisId, peerIdToAddress, 0L, 1).start();
        }
    }
