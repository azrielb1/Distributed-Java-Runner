...
public class RoundRobinLeader extends Thread implements LoggingServer {
    ...
    private final JavaRunnerFollower followerThread;
    ...
    public RoundRobinLeader(ZooKeeperPeerServer peerServer, List<InetSocketAddress> workers, JavaRunnerFollower followerThread) throws IOException {
    ...
    public void run() {
        ...
        Message qMessage = followerThread.getQueuedResponseAndShutdown(); // First shutdown the follower thread
        if (qMessage != null) { // If our follower thread had a queued response, add it to the completed work
            completedWork.put(qMessage.getRequestID(), qMessage);
        }
        try {
