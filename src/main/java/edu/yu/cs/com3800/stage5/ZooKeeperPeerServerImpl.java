...
public class ZooKeeperPeerServerImpl extends Thread implements ZooKeeperPeerServer {
    ...
    private Gossiper gossiperWorker;
    ...
    public void run() {
        try {
            ...
            gossiperWorker = new Gossiper(this, incomingMessages);
            ...
            roundRobinLeader = new RoundRobinLeader(this, workers, javaRunnerFollower);

            // start helper threads
            senderWorker.start();
            receiverWorker.start();
            gossiperWorker.start();
        } catch (IOException e) {
    ...
    public InetSocketAddress getRandomPeer() {
        synchronized (livePeers) {
            int randomIndex = new Random().nextInt(livePeers.size());
            return livePeers.get(randomIndex);
        }
    }
